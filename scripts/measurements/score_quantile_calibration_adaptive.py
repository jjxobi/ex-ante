"""Are the adaptive model's consistency test quantiles uniform, or is the
scorer broken? The adaptive-kernel counterpart to
score_quantile_calibration.py.

D13.4b registers the adaptive kernel as a SEPARATE model scored on identical
windows, never folded into the baseline. This script is therefore a separate
script, not a parametrisation of score_quantile_calibration.py, mirroring it
line for line except for which model's fit() and forecast() are called. That
duplication is deliberate: the two scripts, and the two models, are meant to
stay independently readable and independently rerunnable, the same
"do not couple separate models" choice eq.adaptive's own module docstring
makes about not importing eq.baseline.

This is what DECISIONS.md D13.4c is checked against: the spatial (S) test
rejection rate at the 5 percent level, over the same 26 weekly windows
score_quantile_calibration.py uses for the baseline, is pre-registered to
fall from 46 percent (12 of 26) to between 15 and 30 percent. Falsified below
5 percent or above 40 percent. See DECISIONS.md D13.4b and D13.4c.
"""

import datetime as dt
import statistics
import sys

sys.path.insert(0, "src")
from eq import adaptive, expander, paths, region, score, storage  # noqa: E402
from eq.masked import MaskedCount, compare_counts  # noqa: E402

WINDOW_DAYS = 7
N_WINDOWS = 26
STRATUM = "shallow"


def main() -> int:
    catalogue = storage.read_parquet(paths.SNAPSHOT_DIR / "catalogue-2026-08-05.parquet")
    fixture = storage.read_parquet(
        paths.REPO_ROOT / "tests" / "fixtures" / "catalogue-fit-window.parquet"
    )
    fitted = adaptive.fit(fixture, STRATUM)
    print(f"fitted {STRATUM} stratum on {len(fixture):,} fixture events")
    print(f"adaptive k={fitted.k}, floor={fitted.sigma_floor_km} km, "
          f"ceiling={fitted.sigma_ceiling_km} km")
    print(f"observed sigma range: [{fitted.sigma_km_min:.3f}, {fitted.sigma_km_max:.3f}] km\n")

    latest = max(e["origintime"] for e in catalogue)
    end = dt.datetime(latest.year, latest.month, latest.day, tzinfo=dt.timezone.utc)

    rows = []
    for i in range(N_WINDOWS):
        w_end = end - dt.timedelta(days=WINDOW_DAYS * i)
        w_start = w_end - dt.timedelta(days=WINDOW_DAYS)
        observed = [
            e
            for e in catalogue
            if w_start <= e["origintime"] < w_end
            and region.stratum_for(e["depth"]) == STRATUM
        ]
        separable = adaptive.forecast(fitted, w_start, w_end)
        dense = expander.expand(separable)
        result = score.score(dense, observed, w_start, w_end)
        rows.append((w_start.date(), result))

    print(f"{'window start':<14} {'n used':>7} {'N q':>8} {'S q':>8} {'M q':>8} {'L q':>8}")
    print("-" * 60)
    for start, r in rows:
        print(
            f"{str(start):<14} {r.n_events_used:>7} "
            f"{_q(r, 'number'):>8} {_q(r, 'spatial'):>8} "
            f"{_q(r, 'magnitude'):>8} {_q(r, 'likelihood'):>8}"
        )

    print()
    print("=" * 60)
    print("Expected vs observed, mean over 26 windows (region masked)")
    print("=" * 60)
    mask_ids = {r.expected_count.mask_id for _, r in rows} | {r.observed_count.mask_id for _, r in rows}
    assert len(mask_ids) == 1, f"rows do not share a single region mask: {mask_ids}"
    region_mask = mask_ids.pop()

    mean_expected = MaskedCount(
        sum(r.expected_count.value for _, r in rows) / len(rows), region_mask
    )
    mean_observed = MaskedCount(
        sum(r.observed_count.value for _, r in rows) / len(rows), region_mask
    )
    comparison = compare_counts(mean_expected, mean_observed)
    print(f"  mean expected (in-region) : {comparison.expected:.4f} / week")
    print(f"  mean observed (in-region) : {comparison.observed:.4f} / week")
    print(f"  over-prediction           : {comparison.pct_over_prediction:+.1f}%")

    print()
    print("=" * 60)
    print("Distribution of the spatial test quantile")
    print("=" * 60)
    spatials = [_qf(r, "spatial") for _, r in rows if _qf(r, "spatial") is not None]
    if not spatials:
        print("  no spatial quantiles produced")
        return 1
    below_01 = sum(1 for q in spatials if q < 0.01)
    below_05 = sum(1 for q in spatials if q < 0.05)
    print(f"  windows scored     : {len(spatials)}")
    print(f"  median quantile    : {statistics.median(spatials):.4f}")
    print(f"  S-test rejects at 5% (q < 0.05)  : {below_05} of {len(spatials)}"
          f"  (expected about {0.05*len(spatials):.1f})")
    print(f"  S-test rejects at 1% (q < 0.01)  : {below_01} of {len(spatials)}"
          f"  (expected about {0.01*len(spatials):.1f})")
    print()
    print("=" * 60)
    print("D13.4c pre-registered check")
    print("=" * 60)
    rate = below_05 / len(spatials)
    print(f"  observed 5% rejection rate: {100*rate:.1f}% ({below_05} of {len(spatials)})")
    print(f"  baseline (D13.4a) was     : 46.2% (12 of 26)")
    print(f"  predicted band             : 15% to 30%")
    if rate <= 0.05:
        print("  FALSIFIED (low side): at or below 5%, the grid is not the "
              "limiting factor and the fixed kernel's form was the whole problem")
    elif rate > 0.40:
        print("  FALSIFIED (high side): above 40%, the adaptive kernel bought "
              "essentially nothing; grid resolution confirmed as binding")
    elif 0.15 <= rate <= 0.30:
        print("  INSIDE the pre-registered 15% to 30% band")
    else:
        print("  OUTSIDE the pre-registered 15% to 30% band, but neither "
              "falsification condition met")
    return 0


FIELD = {
    "number": "n_test",
    "spatial": "s_test",
    "magnitude": "m_test",
    "likelihood": "l_test",
}


def _qf(result, name):
    q = getattr(getattr(result, FIELD[name]), "quantile", None)
    if isinstance(q, (tuple, list)):
        q = q[-1]
    return q


def _q(result, name):
    q = _qf(result, name)
    return "n/a" if q is None else f"{q:.4f}"


if __name__ == "__main__":
    raise SystemExit(main())
